...
export default function AdminMenu({ userRole, managedSchoolId }: Props) {
  ...
  console.log('AdminMenu userRole:', userRole); // Debug log

  if (!userRole || (userRole !== 'SUPER_ADMIN' && userRole !== 'SCHOOL_ADMIN')) {
    console.log('AdminMenu returning null due to role check'); // Debug log
    return null;
  }
        ...
        <div className="absolute left-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
          <div className="py-1">
            {userRole === 'SUPER_ADMIN' ? (
              ...
              <Link
                href={`/admin/schools/${managedSchoolId}/edit`}
                className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                onClick={() => setIsOpen(false)}
              >
                Manage School
